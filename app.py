...
.fmt-table {
    ...
    color: #666; margin: 0.3rem 0 0.8rem 0;
}
</style>
...
def is_bot_error(msg: str) -> bool:
    m = msg.lower()
    return any(k in m for k in ("sign in", "bot", "cookies", "confirm your age",
                                 "login required", "not a bot"))

def is_format_error(msg: str) -> bool:
    m = msg.lower()
    return "not available" in m or "requested format" in m


# ── yt-dlp client ladder ───────────────────────────────────────────────────────
#   Ordered from most-permissive to least.
#   ios + android skip the "sign in to confirm" gate more often than web clients.
...
CLIENTS = [
    ...
    {
        # plain web — last resort; most likely to get bot-checked without cookies
...
COMMON = {
    ...
    # Never abort because a single format is unavailable — let the fallback chain work
    "ignoreerrors":      False,
}

# ── Format map ─────────────────────────────────────────────────────────────────
#   Rules:
#   • No hard [ext=mp4] or [ext=m4a] filters — those cause "not available" when
#     YouTube only serves webm for a video.
#   • Height cap only; format_sort + merge_output_format handle containers.
#   • Every chain ends with /best as a guaranteed last resort.
...
def _opts(client: dict, extra: dict, cookie_path: str | None) -> dict:
    o = {**COMMON, **client, **extra}
    if cookie_path:
        o["cookiefile"] = cookie_path
    return o


# ── fetch_info — NO format string, NO caching (avoids stale-cache bugs) ────────

def fetch_info(url: str, cookie_path: str | None) -> dict:
    """
    Extract video metadata only.
    Key fix: do NOT specify 'format' here — let yt-dlp return all available
    formats without validating any particular one. That's what was causing
    "Requested format is not available" on fetch.
    """
    last_err = None
    for client in CLIENTS:
        try:
            opts = _opts(client, {
                "skip_download":    True,
                # Do NOT pass 'format' key here — causes format validation on some videos
                "listformats":      False,
            ...
            msg = str(e)
            if is_bot_error(msg):
                break          # cookies needed — retrying other clients won't help
...
def get_available_formats(info: dict) -> list:
    ...
    for f in reversed(info.get("formats", [])):
        ...
        if key not in seen:
            seen.add(key)
            out.append({
                "format_id": f.get("format_id"), "ext": ext, "height": h,
...
def download_video(url: str, fmt: str, is_audio: bool,
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                opts = _opts(client, {
                    ...
                    # Prefer mp4/h264 but never require it
            ...
            if any(k in msg.lower() for k in
                   ("403", "http error", "timed out", "not available",
                    "format", "unavailable", "fragment")):
                continue
            raise
...
def save_cookie_file(uploaded) -> str | None:
    if uploaded is None:
        return None
    if st.session_state.get("_cookie_name") != uploaded.name:
        old = st.session_state.pop("_cookie_path", None)
        if old and os.path.exists(old):
            os.unlink(old)
        st.session_state["_cookie_name"] = uploaded.name
    if "_cookie_path" not in st.session_state:
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".txt", mode="wb")
        tmp.write(uploaded.getvalue())
        tmp.flush()
        tmp.close()
        st.session_state["_cookie_path"] = tmp.name
    return st.session_state["_cookie_path"]
...
# ── Cookie upload ──────────────────────────────────────────────────────────────
with st.expander("🍪  Upload cookies.txt  (only needed if you see a bot/sign-in error)"):
    st.markdown("""
**Export cookies in ~30 seconds:**

1. Install [**Get cookies.txt LOCALLY**](https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc) (Chrome/Edge)  
...
3. Click the extension → **Export as cookies.txt** → save the file
4. Upload it below ↓

> Cookies are used only for this session and never stored permanently.
""")
    cookie_upload = st.file_uploader("cookies.txt", type=["txt"], label_visibility="collapsed")
    if cookie_upload:
        st.success("✅ Cookies loaded — bot detection bypassed.")

cookie_path = save_cookie_file(cookie_upload if "cookie_upload" in dir() else None)
...
if fetch_btn:
    if not url_input:
        ...
        # Clear stale info so we never show old data for a new URL
            ...
            try:
                ...
                if is_bot_error(err):
                    st.warning(
                        ...
                        "export cookies from youtube.com, upload the file, then retry."
                    )
                elif is_format_error(err):
                    st.info(
                        "Format detection failed. This is usually a temporary YouTube "
                        "issue — try again in a few seconds, or paste the URL again."
                    )
...
if st.session_state.get("info"):
    ...
    if heights:
        st.markdown(
            ...
            f'{"  ·  ".join(f"{h}p" for h in heights[:10])}</div>',
            unsafe_allow_html=True,
        )
    ...
    if st.button("⬇️  Download Now"):
        with st.spinner("Downloading… picking best available quality"):
            try:
                data, fname = download_video(url, fmt_str, is_audio, cookie_path)
                st.session_state.update({
                ...
                if is_bot_error(err):
                    ...
                    st.info(
                        "**Tips:**\n"
                        "- Try **Best available video** — most reliable\n"
                        "- Try **MP3 Audio** for audio-only\n"
                        "- Age-gated / DRM videos require cookies"
                    )
